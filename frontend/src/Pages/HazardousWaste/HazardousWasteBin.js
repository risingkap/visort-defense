...
const HazardousWasteBin = () => {
  ...
  const fetchBinData = async () => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/disposals?binId=ESP32CAM-01&types=Hazardous&startDate=${startDate}&requireDate=true&requireFileSize=true&fields=summary`);
      if (!response.ok) {
      ...
      const filtered = (Array.isArray(data) ? data : []).filter(item =>
        (item.binId || '') === 'ESP32CAM-01' && (item.binType || '') === 'Hazardous'
      );

      const transformedData = filtered.map(item => ({
        id: item._id,
        type: item.binType || 'Hazardous',
        binId: item.binId || '',
  ...
  // Removed status badges in summary view
  ...
  const exportToPDF = () => {
    ...
    // Add title
    doc.setFontSize(16);
    doc.text('Hazardous Waste Bins Report', 14, 22);
    ...
    const tableData = bins.map(bin => [
      bin.id.slice(0, 8) + '...',
      bin.type,
      bin.collectionSchedule
    ]);
    
    // Add table
    autoTable(doc, {
      head: [['Bin ID', 'Type', 'Schedule']],
      body: tableData,
      startY: 40,
      styles: {
        fontSize: 8,
    ...
    // Save the PDF
    doc.save('hazardous-waste-bins-report.pdf');
  };
  ...
  if (loading && !refreshing) return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Loading Hazardous Waste Data</h3>
  ...
  if (!loading && bins.length === 0) return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 text-center max-w-md w-full">
        ...
        <h3 className="text-xl font-bold text-gray-900 mb-2">No Hazardous Waste Bins</h3>
        <p className="text-gray-600 mb-6">No bins have been registered in the system yet.</p>
        <button
          onClick={handleRefresh}
          className="inline-flex items-center px-6 py-3 border-2 border-transparent text-sm font-semibold rounded-xl shadow-lg text-white bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 focus:outline-none transition-all duration-200 hover:shadow-xl"
        >
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-3">
          <div className="flex items-center space-x-3 mb-1">
            <div className="h-9 w-9 bg-gradient-to-br from-red-500 to-red-600 rounded-lg flex items-center justify-center shadow-lg">
            ...
            <div>
              ...
              <p className="text-gray-600 text-sm">Monitor and track hazardous waste bin status</p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-3">
          <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-4">
            ...
            <div className="relative">
              <select
                id="bin-select"
                className="block w-full pl-3 pr-8 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all duration-200 bg-white shadow-sm"
          ...
          <div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden mb-3">
            <div className="bg-gradient-to-r from-red-50 to-red-100 px-4 py-3 border-b border-red-200">
              <div className="flex items-center space-x-3">
                <div className="h-7 w-7 bg-gradient-to-br from-red-500 to-red-600 rounded-lg flex items-center justify-center shadow-lg">
                ...
                <div>
                  ...
                  <p className="text-red-600 font-medium text-sm">Hazardous Waste Container</p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden">
          <div className="bg-gradient-to-r from-gray-50 to-gray-100 px-4 py-3 border-b border-gray-200">
            <div className="flex items-center space-x-3">
              ...
              <h3 className="text-base font-bold text-gray-900">All Hazardous Disposals (ESP32CAM-01)</h3>
            </div>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {bins.map((bin) => (
                  <tr 
                    key={bin.id} 
                    className={bin.id === selectedBin?.id ? 'bg-red-50 border-l-4 border-red-500' : 'hover:bg-gray-50 cursor-pointer transition-colors duration-200'}
          ...
          <div className="bg-gradient-to-r from-gray-50 to-gray-100 px-3 py-2 border-t border-gray-200 flex justify-between items-center">
            ...
            <button
              type="button"
              onClick={exportToPDF}
              className="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-semibold rounded-md shadow-lg text-white bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 focus:outline-none transition-all duration-200 hover:shadow-xl"
            >
              <svg className="w-3 h-3 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mt-2">Delete Hazardous Waste Record</h3>
              <div className="mt-2 px-7 py-3">
                <p className="text-sm text-gray-500">
                  Are you sure you want to delete this hazardous waste record? This action cannot be undone.
                </p>
...
export default HazardousWasteBin;